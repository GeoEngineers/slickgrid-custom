(function ($, window) {
  ...
  /**
   * This manager enables users to copy/paste data from/to an external Spreadsheet application such as MS-Excel® or
   * OpenOffice Calc.
   *
   * Since it is not possible to access directly the clipboard in javascript, the plugin uses a trick to do it's job.
   * After detecting the keystroke, we dynamically create a textarea where the browser copies/pastes the serialized
   * data.
   *
   * @param {Object} options    Options to the plugin.
   * @param {function} options.dataItemColumnValueExtractor   option to specify a custom column value extractor function
   * @param {function} options.dataItemColumnValueSetter      option to specify a custom column value setter function
   * @param {function} options.clipboardCommandHandler        option to specify a custom handler for paste actions
   * @param {boolean} options.includeHeaderWhenCopying        set to true and the plugin will take the name property
   * from each column (which is usually what appears in your header) and put that as the first row of the text that's
   * copied to the clipboard
   * @param {Object} options.bodyElement                      option to specify a custom DOM element which to will be
   * added the hidden textbox. It's useful if the grid is inside a modal dialog.
   * @param {function} options.onCopyInit                     optional handler to run when copy action initializes
   * @param {function} options.onCopySuccess                  optional handler to run when copy action is complete
   * @param {function} options.newRowCreator                  function to add rows to table if paste overflows bottom of table
   * @param {boolean} options.readOnlyMode                    suppresses paste
   * @param {String} options.copiedCellStyleLayerKey          sets the css className used for copied cells. default : "copied".
   * @param {String} options.copiedCellStyle                  Style for copied (Target) cell.
   * @constructor
   */
    ...
    function handleKeyDown(e) {
      ...
      if (!_grid.getEditorLock().isActive() || _grid.getOptions().autoEdit) {
            ...
            for (var rg = 0; rg < ranges.length; rg++){
                ...
                for (var i=range.fromRow; i< range.toRow+1 ; i++){
                    ...
                    if (clipTextRows === "" && _options.includeHeaderWhenCopying) {
                        var clipTextHeaders = [];
                        for (var fc = range.fromCell; fc < range.toCell + 1 ; fc++) {
                            if (columns[fc].name.length > 0)
                                clipTextHeaders.push(columns[fc].name);
                        }
                        clipTextRows.push(clipTextHeaders.join("\t"));
                    }

                    for (var fc2=range.fromCell; fc2< range.toCell+1 ; fc2++){
                        clipTextCells.push(getDataItemValueForColumn(dt, columns[fc2]));
                    }
        ...
        if (!_options.readOnlyMode
            ...
            var emptyTextBox = _createTextBox('');
            
            setTimeout(function(){
                _decodeTabularData(_grid, emptyTextBox);
            }, 100);
